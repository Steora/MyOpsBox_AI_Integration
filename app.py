...
FALLBACK_GEMINI_MODEL = "gemini-3.1-pro-latest"
...
def set_table_borders(table, color="000000"):
    ...
    for border_name in ['top', 'left', 'bottom', 'right', 'insideH', 'insideV']:
        ...
        border.set(qn('w:sz'), '8') # Thickened to 1pt so PDF engines don't erase it
        border.set(qn('w:space'), '0')
        border.set(qn('w:color'), color)
        tblBorders.append(border)
        
    # Remove existing default borders and apply our solid black ones
    ...
    while idx < len(lines):
        ...
        if line.startswith("|"):
            ...
            while idx < len(lines) and lines[idx].strip().startswith("|"):
                ...
                is_separator = all(cell.replace("-", "").replace(":", "").strip() == "" for cell in row_cells)
                if row_cells and not is_separator:
            ...
            if table_rows:
                ...
                set_table_borders(grid_table, color="000000") # Apply strong table-level borders
                
                is_metadata_card = (max_cols == 4)
                
                is_expected_outcomes = False
                if max_cols == 2:
                    first_row_text = "".join([c.lower() for c in table_rows[0]]).replace(" ", "")
                    if "outcome" in first_row_text and "whatitmeans" in first_row_text:
                        is_expected_outcomes = True

                for r_num, row_data in enumerate(table_rows):
                    if r_num == 0:
                        header_str = "".join([c.lower() for c in row_data]).replace(" ", "")
                        is_dummy = ("categorysummary" in header_str) or (header_str.replace("*", "") == "")
                        if is_dummy and not is_expected_outcomes:
                            continue 
                    ...
                    is_header_row = (r_num == 0 and not is_metadata_card and (max_cols != 2 or is_expected_outcomes))
                    if is_header_row:
                    ...
                    for c_num, val in enumerate(row_data):
                        if c_num < len(row_cells):
                            ...
                            # Removed manual cell-level tcBorders here to fix the PDF rendering overlap bug.
                            ...
                            if is_header_row:
                                ...
                                if max_cols == 2:
                                    if c_num == 0:
                                        set_cell_background(cell, "F4EBFA") 
                                        run.font.bold = True
        ...
        if line.startswith("## "):
            ...
            run.font.name = 'Arial'; run.font.size = Pt(14); run.font.bold = True; run.font.color.rgb = primary_color
            idx += 1
            continue
        ...
        # --- BULLET POINTS (Fixed space requirement to prevent false bold triggers) ---
        ...
        # --- STANDARD PARAGRAPHS (Now handles inline bolding!) ---
        p = doc.add_paragraph()
...
def audit_to_docx_buffer(text_content, meta_dict):
    """Dedicated DOCX engine matching the exact MJR Business Advisors PDF replica."""
    ...
    # 1. Italicized Contact and Business Name
    client_contact = meta_dict.get('client_contact', 'Client')
    r1 = p_meta.add_run(f"Prepared for {client_contact}, {client_name}\n")
    r1.font.italic = True
    r1.font.name = 'Arial'
    r1.font.size = Pt(12)
    r1.font.color.rgb = COLOR_TEXT_DARK
    
    # 2. Standard Audit Date
    r2 = p_meta.add_run(f"Audit Date: {meta_dict.get('audit_date', 'TBD')}\n")
    r2.font.name = 'Arial'
    r2.font.size = Pt(10)
    r2.font.color.rgb = COLOR_TEXT_DARK
    
    # 3. Standard Prepared By
    ...
    # Inject a true solid paragraph bottom border via XML
    pPr = p_div._p.get_or_add_pPr()
    pBdr = OxmlElement('w:pBdr')
    bottom = OxmlElement('w:bottom')
    bottom.set(qn('w:val'), 'single')
    bottom.set(qn('w:sz'), '18')  # Thickness of the solid line
    bottom.set(qn('w:space'), '1')
    bottom.set(qn('w:color'), HEX_PURPLE_MAIN)
    pBdr.append(bottom)
    pPr.append(pBdr)
...
if "meetings_list" in st.session_state and st.session_state["meetings_list"]:
    ...
    if selected_calls:
        ...
        with tab1:
            ...
            with st.expander("📝 Required Intake Fields for Title Block", expanded=True):
                col_i1, col_i2 = st.columns(2)
                with col_i1:
                    client_business_name = st.text_input("Client/Business Name", value="")
                    client_contact = st.text_input("Client Contact Name/Title", value="")
                    audit_date = st.text_input("Audit Date", value=datetime.now().strftime('%B %d, %Y'))
                with col_i2:
                    blueprint_date = st.text_input("Blueprint Review Date", value="")
                    prepared_by = st.text_input("Prepared By", value="Patti Zapparolli, Founder")

            if st.button("🔥 Generate Growth Drag Audit Report", type="primary"):
                if not client_business_name or not client_contact:
                    st.warning("Please fill out at least the Client Business Name and Contact fields.")
                else:
                    # Dynamically extract the first name from the client contact field
                    client_first_name = client_contact.split()[0] if client_contact else "the Client"
                    with st.spinner("Diagnosing operational drag and formatting client report..."):
                        try:
                            audit_prompt = f"""
You are an expert Fractional COO acting on behalf of My OpsBox. Thoroughly analyze the attached raw Fathom call transcript and generate a client-facing "Growth Drag Audit Summary Report".

### 🧠 CONTEXTUAL KNOWLEDGE BASE (DYNAMIC SCENARIO MODELING):
Analyze the transcript to determine the client's primary operational drag. 

**IF the primary need is Lead Generation, Prospecting, or Client Acquisition (e.g., MJR Business Advisors):** 
You MUST apply the "HR Hub LV Lead Generation Model" logic to your entire diagnosis:
1. The client is strong at delivery, consulting, advising, or relationship-building. The drag is *not* their expertise.
2. The drag is inconsistent or owner-dependent business development.
3. The business needs a repeatable, visible, manageable outreach lane.
4. Support should help create qualified conversations.
5. The owner should enter the process only when their expertise is most valuable.
6. Growth should be controlled and manageable, not overwhelming.

### 🎯 QUALITY CONTROL & TONE (CRITICAL - "THE CLIENT HEARD US" LAYER):
- **Tone:** Client-facing, clear, operational, warm but professional, plainspoken, and confident. Reflect their actual business situation and language.
- **The Core Goal:** The report must say, in effect: "We heard what you said. Here is where the drag is happening. Here is where to start. Here is the business outcome we are aiming for. The Traction Blueprint Review is where we turn this into the practical plan."
- **Self-Check:** Before generating, ask: "Would the client recognize themselves in this report?" If it sounds like it could apply to any generic consultant, rewrite it to reflect this specific business, specific concern, and specific phrasing.
- **NEVER Sound Like:** Academic, over-engineered, highly technical, a proposal, an internal strategy document, or generic AI consulting language.
- **Phrases to STRICTLY AVOID:** "high-capacity solopreneurship", "passive, inbound-driven business models", "systematic outreach protocol", "data-cleaning protocol", "deploy a centralized tracking sheet", "automated operational function", "client acquisition metrics", "operational risk" (unless central), "specialized support".
- **Preferred My OpsBox Phrasing:** "The business has outgrown owner-only follow-up.", "The prospect list exists, but it is not ready to work yet.", "Outreach needs a simple rhythm that can be tracked and improved.", "The owner should not be spending their best hours on cold prospecting.", "The goal is not a flood of leads. It is a steady flow of well-fit conversations.", "The next step is to turn this into a practical plan."

### ⚙️ STRATEGIC RULES & CONTEXT:
1. **The "Slow Burn" Concept:** Frame lead-generation goals as controlled, targeted, manageable client acquisition. Use phrases like "a steady flow of well-fit prospects that {client_first_name} can realistically develop, serve, and convert." Do not frame it as aggressive lead generation or a "flood of leads."
2. **Inbound to Outbound Shift:** If applicable, explicitly recognize if the client is struggling with the shift from a business where customers came to them to a consulting model where they must actively create opportunities.
3. **Security Constraints:** Security/phishing concerns should be positioned strictly as a "delegation readiness" item (e.g., "Confirm secure access and account protection before support begins outreach") under the 'Where to Start' or 'Current State' sections. Do NOT make security a primary growth drag unless it is the absolute core issue.
4. **Where to Start (High-Level Only):** Keep it high-level. Do NOT over-specify the outreach process (No "Day 1 LinkedIn, Day 3 email"). NO 30-60-90 implementation language, exact scripts, specific cadences, pricing, staffing details, or task-level execution plans.

### 📋 MANDATORY REPORT STRUCTURE (Strict Markdown):
Do NOT output a title block. Start immediately with '## Executive Summary'. Never use ASCII art lines or extra markdown divider artifacts (`---`) outside of tables.

## Executive Summary
[2 short paragraphs summarizing the primary drag, situation, and opportunity. Include the 'inbound to outbound' shift and 'slow burn' pacing if applicable. Prove you listened by reflecting their specific business and concerns.]
...
| Primary Operating Gap | [1-3 sentences on where the bottleneck occurs] |

## Primary Growth Drag Findings
[Provide 3 to 5 findings using exact sub-structure below. Translate problems into clear operational drag.]
### Drag Area 1: [Clear Finding Title]
**What showed up:** [Summary of challenge from transcript, specific to the client's words]
...
## Where to Start
[Short introductory paragraph followed by 3-5 high-level strategic bullet points focusing on immediate relief, list cleaning, tracking, and delegation readiness/security.]

## Expected Business Outcomes
| Outcome | What It Means for {client_first_name} |
| --- | --- |
| A More Consistent Pipeline | [Connect to client's business/emotional reality. e.g., Outreach becomes a steady operating rhythm...] |
| Better Use of Owner's Expertise | [Connect to client's business/emotional reality] |
| Cleaner Lead Visibility | [Connect to client's business/emotional reality] |
| More Manageable Growth | [Frame around the "slow burn" / controlled pacing concept] |
| Lower Financial Pressure | [Connect to client's business/emotional reality] |

## Next Step: Traction Blueprint Review
The **Traction Blueprint Review on {blueprint_date}** will turn these findings into a practical recommended plan. The review will clarify the highest-impact starting priorities, the outreach and handoff structure, how progress should be tracked, and what support will create the best path to traction. The client is already moving in the right direction by identifying the gap before adding more activity. The next step is to align the process, ownership, and measurement so growth can become more consistent, visible, and manageable.

### 🎤 LIVE CALL TRANSCRIPT TEXT TO PROCESS AND ANALYZE:
{raw_transcript_block}
"""
                            sys_inst = "You are a world-class Fractional COO diagnostician acting on behalf of My OpsBox. Write client-facing, plainspoken, empathetic, and operational reports. Do not use generic AI consulting speak."
            ...
            if "compiled_audit" in st.session_state:
                ...
                audit_file_name = f"{safe_b_name} Audit Summary"
